...
def calculate_net_income_jax(
    ...
    function."""
    female_deductions = calculate_ssc_deductions_jax(deductions_spec, female_wage)

    # Check if spouse is present
    male_deductions = calculate_ssc_deductions_jax(deductions_spec, male_wage)
    taxable_income = male_wage + female_wage - male_deductions - female_deductions
    if tax_splitting:
        # Ehegattensplitting
        inc_tax = calculate_inc_tax_jax(
            income_tax_spec, taxable_income / (partner_indicator + 1)
        ) * (partner_indicator + 1)
    else:
        inc_tax = calculate_inc_tax_jax(
            income_tax_spec, female_wage - female_deductions
        ) + calculate_inc_tax_jax(income_tax_spec, male_wage - male_deductions)
...
@jit
def calculate_inc_tax_jax(tax_params, taxable_income):
...
@jit
def calculate_ssc_deductions_jax(deductions_spec, gross_labor_income):
