...
def random_init():
    ...
    init_dict["GENERAL"]["num_periods"] = np.random.randint(8, 12)
    ...
    init_dict["INITIAL_CONDITIONS"]["educ_max"] = 14
    init_dict["INITIAL_CONDITIONS"]["educ_min"] = 10

    init_dict["SIMULATION"]["seed_sim"] = np.random.randint(1000, 9999)
    init_dict["SIMULATION"]["num_agents_sim"] = np.random.randint(750, 1250)
    ...
    init_dict["PARAMETERS"]["optim_paras"] = np.append(
        gamma0, np.append(gamma1,np.append(g_s, np.append(delta, np.append(theta, sigma)))
    )).tolist()
