...
def simulate(model_params_init_file_name, model_spec_init_file_name, is_expected = True):
    ...
    states, indexer, covariates, emaxs = pyth_solve(model_params, model_spec, is_expected)

    # Simulate agents experiences according to parameters in the model specification
    df = pyth_simulate(model_params, model_spec, states, indexer, emaxs, covariates, is_expected=False)
